...
class Enemy:
    def __init__(self):
        _types = ['CloroxChan', 'TidePod', 'Laundry Sauce', 'TidePodChan', 'EarthChan']
        _weps = ['Dead memes', 'Clorox', 'Laundry sauce', 'TidePods']
        self.hp = randint(5,10)
        self.name = _types[randint(0,4)]
        self.wep = _weps[randint(0,3)]
        self.dam = randint(1,7)
...
def attack(player, numbered_opponents):
    ...
    while _condition == False:
        print('What are you fighting with')
        weapon = str(input('- '))
        weapon = weapon.lower()
    ...
    for spell, damage in player.spells:
        if spell == 'healing spell':
            print('- Healing spell ( Got the heals )')
